...
x=pd.merge(df,salaries , left_on='ID_y' , right_on='EMP_ID') //Merging the the tables,above//
                                                             //merged table's l wiht the salaries table//

x=data.groupby('[NAME_X]')['AMT'].mean()  //Performing grouping and calculating mean of the 'AMT' column for each unique//
...
x.sort_values(ascending=False).head(3) // this sorts the values in the x Series in descending order (ascending=False)/
                                       //then retrieves the top 3 values using the method head(3) .//       
...
output = 15 because 25-20=5 , which is 5+4+3+2+1.
